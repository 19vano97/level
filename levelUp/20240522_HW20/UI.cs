...
public class UI
{
    ...
    public static void PrintNextMainMenu(Group[] groupList, string[] idRecords, Supervisor[] supervisorList)
    {
        ...
        do
        {
            ...
            switch (key)
            {
                ...
                case ConsoleKey.A:
                    System.Console.WriteLine();
                    Group groupToAdd = BL.CreateEmptyGroup(supervisorList);
                ...
                case ConsoleKey.D:
                    ...
                    {
                        System.Console.WriteLine("The {0} group has been succesfully removed", groupToDelete.groupName);
                    }
                    else
                    {
    ...
    public static void EditGroupUI(Group[] groupList, Group groupToEdit, string[] idRecords, Supervisor[] supervisorList)
    {
        ...
        do
        {
            ...
            switch (key)
            {
                case ConsoleKey.A:
                    System.Console.WriteLine();
                    groupToEdit.groupOfStudents = BL.CreateStudentsArray(groupToEdit, idRecords);
                    groupToEdit.avrMarkOfGroup = BL.AvgMarkOfGroup(groupToEdit);
                ...
                case ConsoleKey.T:
                    ...
                    BL.UpdateGroups(groupList, groupToEdit, groupToEdit);
                    System.Console.WriteLine("The group has been succefully moved to {0} year", groupToEdit.currentYear);
                    Thread.Sleep(DELAY);
                    Console.Clear();
                    EditGroupUI(groupList, groupToEdit, idRecords, supervisorList);
                    break;
    ...
    public static Group UseGroupFromList(Group[] groupList = null)
    {
        ...
        {
            System.Console.WriteLine("{0}. {1}", i, groupList[i].groupName);
        }
    ...
    public static void PrintGroup(Group groupPrint)
    {
        ...
        System.Console.WriteLine("Start date of enrollment: {0}", groupPrint.startDate.ToString("yyyy-MM-dd"));
       ...
       if (groupPrint.groupOfStudents != null)
       {
            for (int i = 0; i < groupPrint.groupOfStudents.Length; i++)
            {
                System.Console.WriteLine("{0}. {1} {2}, IdRecord: {3}, Avarage mark: {4}", i, groupPrint.groupOfStudents[i].lastName,
                                            groupPrint.groupOfStudents[i].firstName, groupPrint.groupOfStudents[i].idRecord,
                                            groupPrint.groupOfStudents[i].avgMark);
            }
       }
       else
       {
    ...
    public static Student ChooseStudent(Group studentGroup)
    {
        ...
        {
            System.Console.WriteLine("{0}. {1} {2}, IdRecord: {3}", i, studentGroup.groupOfStudents[i].lastName,
                                        studentGroup.groupOfStudents[i].firstName, studentGroup.groupOfStudents[i].idRecord);
        }

        int studentIndex = EnterIntValue("number of the student");

        return studentGroup.groupOfStudents[studentIndex];
    }
